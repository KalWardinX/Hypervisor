	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xEB)
			{
				exit_count_in++;
				char* p = (char*)vcpu->kvm_run;
				uint32_t* target = (uint32_t*) (p+vcpu->kvm_run->io.data_offset);
				*target = exit_count;
				continue;
			}
			...
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xED)
			{
				exit_count_out++;
				char* p = (char*)vcpu->kvm_run;
				fprintf(stdout, "%s", vm->mem + *(uint32_t*)(p + vcpu->kvm_run->io.data_offset));
				fflush(stdout);
				continue;
			}
			...
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xED)
			{
				...
				char* p = (char *)vcpu->kvm_run;
				char s[64];
				// printf("Here writing to str->  in: %u, out: %u\n", exit_count_in, exit_count_out);
				sprintf(s, "IO in: %u\nIO out: %u\n", exit_count_in, exit_count_out);
				sprintf(vm->mem + offset, "%s", s);
				uint32_t* target = (uint32_t*)(p+vcpu->kvm_run->io.data_offset);
				*target = offset;
				offset+=strlen(s)+1;
				
				// printf("Here writing to str");
				fflush(stdout);
				continue;
			}
			...
			else if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEF)
			{
				...
				// printf("was here\n");
				// printf("addr : %p, size : %u\n", p+vcpu->kvm_run->io.data_offset, vcpu->kvm_run->io.size);
				gva_hva.gva =  *((uint32_t*)(p+vcpu->kvm_run->io.data_offset));


				struct kvm_translation t;
				memset(&t, -1, sizeof(t));
				t.linear_address = gva_hva.gva;
				ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &t);
				// printf("gva: %llu\nhva: %llu\nvalid: %u\n", t.linear_address, t.physical_address, t.valid);
				if(t.valid){
					
					gva_hva.hva = (uint32_t)(intptr_t)vm->mem + t.physical_address;
				}
				else{
					fprintf(stderr, "Invalid GVA\n");
					gva_hva.hva = 0;
				}
				continue;
			}
