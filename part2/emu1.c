...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
 // if (pthread_create(&(vm1->vcpus->vcpu_thread), (const pthread_attr_t *)NULL, vm1->vcpus->vcpu_thread_func, vm1) != 0)
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            printf("VMFD: %d KVM_EXIT_IO\n", vm->vm_fd);
            if(val%3==1 && vm->vcpus->kvm_run->io.port == 0x11){
                char* p = (char*)vm->vcpus->kvm_run;
                printf("VMFD: %d Consuming the number\n", 
                vm->vm_fd);
                *(int*)(p+vm->vcpus->kvm_run->io.data_offset) = data;
            }
            else if(val%3==2 && vm->vcpus->kvm_run->io.port == 0x12){
                char* p = (char*)vm->vcpus->kvm_run;
                printf("VMFD: %d Consumed value: %d\n", 
                vm->vm_fd, *(int*)(p+vm->vcpus->kvm_run->io.data_offset));
            }
            else if(val%3==0 && vm->vcpus->kvm_run->io.port == 0x10){
                char* p = (char*)vm->vcpus->kvm_run;
                data = *(int*)(p+vm->vcpus->kvm_run->io.data_offset);
