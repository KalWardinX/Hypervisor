...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (1)
    {
        ...
        case KVM_EXIT_IO:
            printf("VMFD: %d KVM_EXIT_IO\n", vm->vm_fd);
            if((val%9==3 || val%9==5 || val%9==7) && vm->vcpus->kvm_run->io.port == 0x11){
                char* p = (char*)vm->vcpus->kvm_run;
                // printf("VMFD: %d Consuming the number\n", vm->vm_fd);
                *(int*)(p+vm->vcpus->kvm_run->io.data_offset) = data[(val-3)/2];
            }
            else if((val%9==4 || val%9==6 || val%9==8) && vm->vcpus->kvm_run->io.port == 0x12){
                char* p = (char*)vm->vcpus->kvm_run;
                printf("VMFD: %d Consumed value: %d\n", 
                vm->vm_fd, *(int*)(p+vm->vcpus->kvm_run->io.data_offset));
            }
            else if((val%9==0 || val%9==1 || val%9==2) && vm->vcpus->kvm_run->io.port == 0x10){
                char* p = (char*)vm->vcpus->kvm_run;
                data[val] = *(int*)(p+vm->vcpus->kvm_run->io.data_offset);
