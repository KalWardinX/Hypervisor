...
int run_vm(struct vm *vm1, struct vm *vm2, struct vcpu *vcpu1, struct vcpu *vcpu2, size_t sz)
{
	...
	uint8_t val = 0;
	// uint32_t data[5] = {0};
	uint32_t* prod_buf = NULL;
	uint32_t* cons_buf = NULL;
	bool hyper_flag = false;
	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			// receive buffer address
				...
				char *p = (char*)vcpu->kvm_run;
				uint32_t* buf = (uint32_t*)(vm->mem + *(uint32_t*)(p + vcpu->kvm_run->io.data_offset));
				if(val == 0){
					prod_buf = buf;
					val = 1;
					// printf("get producer addr\n");
					// fflush(stdout);
				}
				else {
					cons_buf = buf;
					val = 0;
					// printf("get consumer addr\n");
					// fflush(stdout);
				}
				continue;
			}
